...
async function seed() {
  ...
  console.log('🌱 Starting BitCommerce HRM Demo Data Seeder...');
  ...
  const store = await storeRepo.findOne({ where: { slug: STORE_SLUG } });
  if (!store) {
    console.error(`❌ Store "${STORE_SLUG}" not found. Run "npm run seed" first to create the demo merchant/store.`);
    process.exit(1);
  }
    ...
    if (!emp) {
      emp = await employeeRepo.save(
        employeeRepo.create({
          ...
          email: i === 0 ? SELF_SERVICE_EMAIL : `${def.fullName.toLowerCase().replace(/\s+/g, '.')}@mydiagnostic.example`,
  ...
  // ─── Self-service demo login (employee[0]) ─────────────────────
  console.log('🔑 Seeding a ready-to-use self-service login...');
  ...
  // ─── Attendance (past 10 days, active employees) ───────────────
  ...
  // ─── Shifts + Roster ────────────────────────────────────────────
  console.log('⏰ Seeding Shifts and Roster...');
  const shiftDefs = [
  ...
  for (let d = 0; d < 5; d++) {
    ...
    for (const emp of activeEmployees.slice(0, 10)) {
      ...
      if (exists) continue;
      const shift = pick(shifts.slice(0, 2)); // mostly Morning/Evening for a normal office week
  ...
  // ─── Salary Structures (skip 3 employees on purpose) ───────────
  console.log('💰 Seeding Salary Structures...');
  const skipSalaryForIdx = new Set([2, 7, 13]); // includes the terminated employee (idx 7)
  ...
  // ─── Payroll Runs (last month PAID, this month DRAFT) ──────────
  ...
  // ─── Tax Slabs (illustrative TEST data only — not real NBR figures) ──
  console.log('🏛️ Seeding illustrative test Tax Slabs (NOT real tax figures — for local testing only)...');
  ...
  console.log('');
  console.log('🎉 HRM Demo Data Seeding Completed Successfully!');
  console.log(`   Departments: ${departments.length}, Employees: ${employees.length}`);
  console.log(`   Self-service login ready — email: ${SELF_SERVICE_EMAIL}, password: ${SELF_SERVICE_PASSWORD}`);
  console.log('');
