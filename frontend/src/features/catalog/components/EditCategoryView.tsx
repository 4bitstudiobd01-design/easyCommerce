...
export function EditCategoryView({ categoryId }: EditCategoryViewProps) {
  ...
  useEffect(() => {
    if (category && !isInitialized) {
      ...
      setShowInStorefront(category.showInStorefront ?? true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateCategory({ id: categoryId, data: payload }).unwrap();
      toast.success(`Category "${result.name}" updated successfully!`);
      setIsDirty(false);
      router.push(`/dashboard/categories/${categoryId}`);
    } catch (err: any) {
      const message = err?.data?.message || err?.message || 'Failed to update category';
  ...
  if (isLoadingCategory) {
    return (
      <div className="max-w-7xl mx-auto py-16 text-center space-y-3">
        ...
        <p className="text-xs font-bold text-slate-700">Loading category details...</p>
      </div>
    );
  }
      ...
      <div className="max-w-xl mx-auto py-16 text-center space-y-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500">
            The category you are trying to edit does not exist or belongs to another store.
          </p>
        </div>
        <Link
