...
import { Wallet, RefreshCw, Edit2, AlertCircle, Sparkles, CheckCircle } from 'lucide-react';
import { TableRowSkeleton } from '@/components/ui/Skeleton';
import { Employee, SalaryStructure, useGetSalaryStructuresQuery, useSeedPayrollDemoDataMutation } from '../api/hrmApi';
...
function grossOf(s: SalaryStructure) {
  return (
    ...
    Number(s.houseRentAllowance) +
    Number(s.medicalAllowance) +
    Number(s.conveyanceAllowance) +
...
export function SalaryStructuresTable() {
  ...
  const handleSeedDemoData = async () => {
    ...
    try {
      ...
      setStatusMessage({
        type: 'success',
        text: `Successfully seeded ${res.salaryStructuresCreated} salary structures and ${res.employeesCreated} employees!`,
      });
      refetch();
    } catch (err: any) {
      setStatusMessage({
        type: 'error',
        text: err?.data?.message || err?.message || 'Failed to seed demo data.',
      });
    }
  };
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={isSeeding}
            className="px-4 py-2.5 rounded-xl border border-blue-200 bg-blue-50/70 text-blue-700 hover:bg-blue-100 text-xs font-bold transition flex items-center gap-2 disabled:opacity-50"
            title="Seed dummy demo employees and salary structures"
          >
            <Sparkles className={`w-4 h-4 ${isSeeding ? 'animate-spin' : 'text-blue-600'}`} />
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-400">
                    ...
                    <p className="text-xs text-slate-500 mt-1">Add employees or seed demo salary records.</p>
                    <div className="mt-4 flex items-center justify-center gap-3">
                      <button
                        onClick={handleSeedDemoData}
                        disabled={isSeeding}
                        className="px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 transition inline-flex items-center gap-2"
                      >
