    ...
    const payload = {
      ...
      rules: {
        ...
        conditions: conditions.map((c) => ({
          ...c,
          value: typeof c.value === 'string' && !isNaN(Number(c.value)) && c.field !== 'status' && c.field !== 'source' ? Number(c.value) : c.value,
        })),
      },
    };
        ...
        <div className="p-5 border-b border-slate-200/80 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Define dynamic SQL rules for customer grouping</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5 text-xs">
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                required
                className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Customers with total spend ≥ ৳10,000 or > 2 orders"
                className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
              />
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-4">
            <div className="flex items-center justify-between">
              <span className="font-extrabold text-xs text-slate-800 uppercase tracking-wider">Rule Conditions</span>
              ...
              {conditions.map((cond, idx) => (
                <div key={idx} className="flex items-center gap-2 bg-white p-2.5 rounded-xl border border-slate-200 shadow-2xs">
                  ...
                  <select
                    ...
                    className="flex-1 p-2 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-800 focus:outline-none"
                  >
                    <option value="ordersCount">Total Orders</option>
                    <option value="totalSpent">Total Spent (BDT)</option>
                    <option value="daysSinceLastOrder">Days Since Last Order</option>
                    <option value="status">Customer Status</option>
                    <option value="source">Customer Source</option>
                  </select>
                  ...
                  <select
                    ...
                    className="p-2 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-800 focus:outline-none"
                  >
                  ...
                  {cond.field === 'status' ? (
                    ...
                    <input
                      ...
                      placeholder="Value"
                      required
                      className="w-28 p-2 bg-slate-50 border border-slate-200 rounded-lg font-bold text-slate-900 focus:outline-none"
                    />
                  )}
            ...
            <button
              ...
              className="w-full py-2 bg-white border border-dashed border-slate-300 hover:border-blue-500 text-blue-600 font-bold rounded-xl transition-all flex items-center justify-center gap-1.5"
            >
          ...
          <div className="bg-indigo-50/60 rounded-2xl p-4 border border-indigo-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-extrabold text-indigo-900 text-xs">Estimated Matching Customers:</span>
            </div>
            ...
            {isPreviewing ? (
              ...
              <span className="font-black text-indigo-700 text-base">
                {previewCount !== null ? `${previewCount} customers` : '—'}
              </span>
            )}
          </div>
