...
function getOverallRisk(data?: FraudCheckResult): RiskLevel {
...
function riskBadgeClasses(level: RiskLevel, hasData: boolean): string {
        ...
        <div className="w-full max-w-md bg-white rounded-[28px] shadow-2xl border border-slate-100 overflow-hidden">
          <div className="p-6 pb-4 flex items-center justify-between border-b border-slate-100">
            <div>
              ...
              {label && <p className="text-xs text-slate-400 font-medium mt-0.5">{label}</p>}
            </div>
            <button
              onClick={onClose}
