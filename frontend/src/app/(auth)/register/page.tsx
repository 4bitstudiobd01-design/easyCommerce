  ...
  return (
    <div className="h-screen overflow-hidden bg-slate-100 text-slate-900 font-sans p-0 sm:p-6 lg:p-8">
      <div className="w-full h-full bg-white rounded-none sm:rounded-3xl shadow-xl shadow-slate-300/40 overflow-hidden grid grid-cols-1 lg:grid-cols-[minmax(0,48fr)_minmax(0,52fr)]">
        ...
        <div className="hidden lg:flex flex-col bg-[#F6F8FF] relative overflow-hidden pl-12 xl:pl-16 pr-10 pt-8 pb-0">
          ...
          <div className="relative z-10 flex flex-col h-full">
            {/* Logo */}
            <Link href="/" className="flex items-center gap-3 mb-5 shrink-0">
            ...
            {/* Trust pill */}
            <div className="inline-flex items-center gap-2 self-start h-7 px-3 rounded-full bg-blue-50 border border-blue-100 mb-4 shrink-0">
              <Heart className="w-3 h-3 text-blue-600 fill-blue-600" />
              <span className="text-[11px] font-bold text-blue-700">Trusted by 1,000+ merchants</span>
            </div>

            {/* Headline */}
            <h1 className="text-[26px] font-extrabold tracking-tight leading-[1.2] mb-2.5 shrink-0">
              Start your eCommerce <br />
              <span className="text-blue-600">journey today</span>
            </h1>
            <p className="text-slate-600 text-[12px] font-medium leading-relaxed max-w-[340px] mb-4 shrink-0">
              Create your BitCommerce merchant account and launch your online store in minutes.
            </p>

            {/* Highlights */}
            <div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-4 shrink-0">
              {HIGHLIGHTS.map(({ icon: Icon, title, description, className }) => (
                <div key={title} className="flex gap-3">
                  <div className={`p-2 rounded-xl shrink-0 h-fit ${className}`}>
                    <Icon className="w-3.5 h-3.5" strokeWidth={2} />
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-900 text-[12.5px] mb-0.5">{title}</h3>
            ...
            <div className="flex-1 min-h-0 flex items-end justify-center">
              <img
                ...
                className="w-full h-full max-w-full object-contain object-bottom"
              />
            </div>
          </div>
        </div>
