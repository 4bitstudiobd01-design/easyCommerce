...
export function ExportSubscriptionsModal({
  isOpen,
  onClose,
  totalSubscriptionsCount,
}: ExportSubscriptionsModalProps) {
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      ...
      toast.success(
        `Export complete! Downloaded subscriptions (${format.toUpperCase()}) with ${totalSubscriptionsCount.toLocaleString()} records.`
      );
      onClose();
    }, 1000);
  };
      ...
      <div className="relative bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl z-10 animate-in fade-in zoom-in-95 duration-150 border border-slate-100">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900">Export Subscriptions</h3>
              <p className="text-xs text-slate-500">
                Export all {totalSubscriptionsCount.toLocaleString()} subscription entries
              </p>
            </div>
          </div>
          <button
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div>
                  ...
                  <span className="text-[11px] text-slate-400">Raw tabular export</span>
                </div>
              </button>
